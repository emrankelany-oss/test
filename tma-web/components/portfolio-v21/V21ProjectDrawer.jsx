...
/* =====================================================================
   V21ProjectDrawer — right-side slide-in panel showing a project's
...
export default function V21ProjectDrawer() {
  ...
  return createPortal(
    <>
      <div
        className={`v21pd-backdrop${isOpen ? " is-open" : ""}`}
        onClick={close}
        aria-hidden={!isOpen}
      ...
      <aside
        ref={panelRef}
        className={`v21pd-panel${isOpen ? " is-open" : ""}`}
        role="dialog"
        aria-modal="true"
        aria-hidden={!isOpen}
        aria-labelledby={project ? "v21pd-title" : undefined}
        tabIndex={-1}
      >
        {project && (
          <>
            <header className="v21pd-head">
              <div className="v21pd-eyebrow">
            ...
            {project.hero && (
              <div className="v21pd-hero">
                <img src={project.hero} alt="" loading="eager" decoding="async" />
              </div>
            )}

            <div className="v21pd-body">
              <h2 id="v21pd-title" className="v21pd-title">
                {project.title}
              </h2>
              {project.tagline && (
                <p className="v21pd-tagline">{project.tagline}</p>
              )}

              {project.services?.length > 0 && (
                <ul className="v21pd-services">
              ...
              {project.intro && <p className="v21pd-intro">{project.intro}</p>}

              {project.deep && (
                <>
                  {project.problem?.length > 0 && (
                    <section className="v21pd-section">
                      <h3>The Problem</h3>
                      <ul>
                  ...
                  {project.results?.length > 0 && (
                    <section className="v21pd-section">
                      <h3>The Results</h3>
                      <ul className="v21pd-results">
                        {project.results.map((r, i) => (
                          <li key={i} className="v21pd-result">
                            <span className="v21pd-result-metric">
                              {r.metric}
                            </span>
                            <span className="v21pd-result-label">{r.label}</span>
                          </li>
                        ))}
                      </ul>
                    </section>
