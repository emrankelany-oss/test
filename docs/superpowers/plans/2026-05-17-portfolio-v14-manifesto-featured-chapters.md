...
**Regression note — broader than first scoped (RECONCILED post-execution).** Deleting the probes invalidates stale scene references in **three** SP-0/1/2A specs plus two timing assumptions. All are intent-preserving consequences of the *sanctioned* SP-2B restructure; none weaken an assertion. Applied fixes:

- `v14-reduced-motion.spec.js`, `v14-transition.spec.js`, `v14-universe.spec.js`: `data-scene="probe-b"` → `data-scene="zid"` (new last scene; same "late scene reachable" intent). `v14-reduced-motion.spec.js` also `data-scene="probe-a"` → `data-scene="film"` (new first scene; same "scene visible on load" intent).
- `v14-transition.spec.js` Test 2 ("seam blur velocity-coupled & bounded"): it hardcoded `scrollTo(1675)` — a y that sat in the *old* probe-a→film seam. SP-2B made the 8-viewport `film` the first scene, so y=1675 is now mid-film (no seam) → `peak=0`. Replaced the hardcoded jump with the file's own geometry-independent sweep (capture peak blur over a full `scrollTo→settle` sweep, the exact pattern Test 1 uses); the assertion `0 < peak <= 27` is unchanged. The transition *engine* is not regressed — Test 1 (`regions>=2`, whole-page sweep) and the chapters e2e pass.
- `v14-chapters.spec.js` Test 1 sweep: a fixed 140ms per-step settle let a narrow act band (observed: `zid:solution`) slip between 280px steps under cumulative dev-server Lenis lag (flaky only after several prior heavy specs in one serial run; passed 2/2 standalone). Replaced the fixed settle with **poll-to-arrival** (wait until `window.scrollY` is within ~80px of target or bottom, capped, then a short rAF-paint wait) so no band can be skipped regardless of load. All assertions identical; verified 2/2 twice + 11/11 full-suite.

Any OTHER failure (kernel fps, universe frame advance, transition `regions>=2`) would be a real regression → STOP and report BLOCKED with evidence (do not mask). All five fixes above are committed in `372ece6`; full v14 e2e is **11/11** and unit **64/64**.
